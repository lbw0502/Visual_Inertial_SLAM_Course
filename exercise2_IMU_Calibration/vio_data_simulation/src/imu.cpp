...
// eulerRates2bodyRates: inertial frame to body frame
...
MotionData IMU::MotionModel(double t)
{
    ...
    MotionData data;
    // param
    // object simulates a ellipse trajectory on x-y plane, a sinus trajectory along z-axis
    float ellipse_x = 15;
    float ellipse_y = 20;
    float z = 1;

    // 20 * K = 2pi
    // every 20s, the object completes a ellipse trajectory
    float K = M_PI/ 10;    
    float K1 = 10;          // the frequency of z-axis is k1 times of x,y axis

    // translation
    // twb:  body frame in world frame
    Eigen::Vector3d position( ellipse_x * cos( K * t) + 5, ellipse_y * sin( K * t) + 5,  z * sin( K1 * K * t ) + 5);

    // first order derivative of position
...
// read generated IMU data
// calculate the integration to compute the trajectory
...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        Eigen::Vector3d dtheta_half =  imupose_last.imu_gyro * dt /2.0;
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        // Eigen::Quaterniond dq;
        // Eigen::Vector3d dtheta_half = (imupose.imu_gyro + imupose_last.imu_gyro)*0.5 * dt /2.0;
        // dq.w() = 1;
        // dq.x() = dtheta_half.x();
        // dq.y() = dtheta_half.y();
        // dq.z() = dtheta_half.z();
        ...
        // Eigen::Vector3d acc_w = 0.5*( Qwb*(imupose_last.imu_acc) + Qwb_new*(imupose.imu_acc) + 2*gw);
        // Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
        // Vw = Vw + acc_w * dt;
        // Qwb = Qwb_new;
