...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    if (frame_count != 0)
    {
        ...
        // prediction of PVQ (under world frame)
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double header)
{
    ...
    // check the disparity of current image
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))

        // disparity is large enough, we can set it as key frame and marginalize an old key frame
        marginalization_flag = MARGIN_OLD;
    else
        // disparity is not large enough, last frame should be marginalized
...
void Estimator::problemSolve()
{
    ...
    // add extrinsic vertex to graph, this vertex will be always used, so we add it at first
    shared_ptr<backend::VertexPose> vertexExt(new backend::VertexPose());
    {
        Eigen::VectorXd pose(7);
        pose << para_Ex_Pose[0][0], para_Ex_Pose[0][1], para_Ex_Pose[0][2], para_Ex_Pose[0][3], para_Ex_Pose[0][4], para_Ex_Pose[0][5], para_Ex_Pose[0][6];
    ...
    // add camera poses verticies and body verticies
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
        shared_ptr<backend::VertexPose> vertexCam(new backend::VertexPose());
        Eigen::VectorXd pose(7);
    ...
    // prior
    {
        if (Hprior_.rows() > 0)
        {
            ...
            // extend the prior matrix to enable new pose
...
void Estimator::backendOptimization()
{
    ...
    // construct a solver
    problemSolve();
    // process the freedom problem after the optimization
    double2vector();
